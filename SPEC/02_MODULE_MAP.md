...
- **個人 iCloud 設定**：
    - **位置**：個人 `SetupWindow` 設定分頁中。
    - **權限**：開放給所有人員（權限 1-5）設定個人隱私。

### 3. 設定歸屬修正 (Privacy & Decoupling)
- **目標**：確保管理員報表中心僅處理「數據監看」，個人隱私設定（如 iCloud URL）回歸個人端。
- **規格**：
    - **[DELETE] 管理員中心**：從 `adminDashboard.html` 移除 iCloud 設定面板與相關 IPC 調用。
    - **[STABLE] 個人設定視窗**：由 `setupWindow.js` 統一管理 iCloud 網址輸入。
    - **[ENHANCE] 統計中心**：維持點擊標籤跳轉 `setupWindow.js` 的導航邏輯，確保操作路徑正確。

### 2. 統計中心「標籤直達」設定 (UX Optimization)
- **目標**：看到 ❌ 提示時，可立即點擊進入設定。
- **規格**：
    - **UI 升級**：iCloud 狀態標籤增加懸停手勢 (`cursor: pointer`) 與提示。
    - **功能跳轉**：點擊標籤直接發送 IPC 指令，調起 `SetupWindow` 並定位至設定頁。
