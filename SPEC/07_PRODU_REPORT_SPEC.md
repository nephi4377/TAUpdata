...
### 2.2 報表提交 (AppCore & API Bridge)
*   **模組**: `appCore.js` (驅動器), `apiBridge.js` (執行者)
*   **觸發時機**: 
    - **定時**: **每 60 分鐘** 自動觸發一次 `submitTodayReport`。
    - **目的**: 數據補位防禦。確保即便同仁在 18:00 前提早關機，系統也已完成最近一小時的數據備份。
    - **手動**: 使用者於看板中點擊「✅ 打卡」或手動觸發。
*   **彙總邏輯 (Aggregation)**:
