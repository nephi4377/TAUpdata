...
const { versionService } = hotReloader.loadModuleSafely('versionService', './src/versionService');
...
function setupAutoUpdater() {
    ...
    app.on('check-for-updates-manual', async () => {
        if (app.isPackaged) {
            isManualCheck = true;
            const updated = await patchUpdater.checkForUpdates(true);
            if (!updated) {
                // 交由 autoUpdater 中處理
...
async function restartAppServices() {
    ...
    // 5. 取得更新後的有效版本
    const effectiveVersion = versionService.getEffectiveVersion();
    const baseVersion = versionService.getBaseVersion();

    log.info(`[Main] 軟重啟完成，當前有效版本: v${effectiveVersion} (基礎版: v${baseVersion})`);

    // 檢查這次軟重啟是不是因為我們剛套用了 "尚未通知過的" 新版本？
    const lastNotifiedPatchStr = configManager.get('lastNotifiedPatch') || '';

    // 如果有效版本大於基礎版本，且尚未通知過
    if (versionService.compareVersions(effectiveVersion, baseVersion) > 0 &&
        versionService.compareVersions(effectiveVersion, lastNotifiedPatchStr) > 0) {

        console.log(`[Main] 增量補丁 v${effectiveVersion} 已加載套用成功！`);

        // 使用系統原生通知取代彈窗，不再強制中斷使用者操作
        if (Notification.isSupported()) {
            new Notification({
                title: '生產力助手更新完成',
                body: `增量補丁 v${effectiveVersion} 已生效`
            }).show();
        }

        // 記憶已通知過的版本
        configManager.set('lastNotifiedPatch', effectiveVersion);
    }
}
